...
def plotBarStacked(source_path):
    ...
    # fp = matplotlib.font_manager.FontProperties(family='JasmineUPC', size=24)
    ...
    ax = df.set_index('TYPE', append=True)['VOL'].unstack().plot.bar(stacked=True, figsize=(10, 8))

    # plt.legend(('งานหลัก', 'งานเหมา'), loc='best', prop={'family':'JasmineUPC', 'size':20})
    ...
    i_count = 1
    # count_row = calendar.monthrange(int(curr_year), int(curr_date_no))[1]
    count_row = int(curr_date_no)

    for p in ax.patches:
        b = p.get_bbox()
        # print('b.y0 = {}, b.y1 = {}, i_count = {}, count_row = {}'.format(b.y0, b.y1, i_count, count_row))
...
def main():
    ...
    filename = plotBarStacked(dfltVal[0])
